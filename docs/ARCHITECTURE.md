...
| `/runs` | **Shipped INERT — Phase 3c.35 / Phase 4 partial.** Job history table + "Run now" panel (per-job manual triggers) + HTMX expand-row for `details_json`. Reads the new `job_runs` table (orchestrator-level, distinct from the per-step `run_log` table that ingest / enrich / cluster / synthesis already write to). APScheduler activation is env-gated (`SCHEDULER_ENABLED=1` + uvicorn restart); manual trigger endpoints always work regardless of flag. Not added to sidebar nav (Phase 3c.35 scope cut). |
