...
- [x] **APScheduler jobs: daily ingest, weekly synthesis.** Shipped 2026-05-27 (inert); **retimed + chained 2026-06-10 (still inert)**. `app/main.py` lifespan creates a `BackgroundScheduler` only when `SCHEDULER_ENABLED=1`. Daily cron now `CronTrigger(hour=23, minute=0)` local (was `hour=7`) so the brief is ready in the morning. The standalone `Mon 07:30` weekly cron was **removed**; the weekly is now **chained inline at the end of each daily run** via `_previous_week_needs_synthesis()` → `run_weekly_extension(week_id=...)` (RLock re-entrant), guaranteeing it runs AFTER that night's ingest + self-healing a missed run. `max_instances=1`, `coalesce=True`. Single `threading.RLock` in `app/services/jobs.py` serializes all paths. See DECISIONS 2026-05-27 "Phase 4 automation locked to single-process APScheduler" + 2026-06-10 Decisions 1–2.
- [x] **Catch-up logic on app startup.** Shipped 2026-05-27 (inert); **simplified 2026-06-10**. Daily overdue if >24h since last `started_at` (or crashed mid-flight). The weekly branch (`_is_weekly_overdue` + weekly constants) was removed — the daily chain now covers weekly catch-up. Lock-miss persists `status='skipped'`.
- [x] **Nightly pipeline must not run full-corpus backfills (cost-critical).** Shipped 2026-06-10. Region backfill on the daily is scoped to items enriched *this run* (`Enrichment.created_at >= run_started AND region_focus IS NULL`), not the whole untagged corpus (was re-running ~10,495 Haiku calls/night → re-billed forever). `retry_failed`/2nd enrich pass (`enrich_after_fetch`) re-enriches ONLY items that just got article bodies (`enrich_pending(item_ids=...)`), not the whole skipped backlog (was re-running Whisper over the entire YT backlog, ~55 min stall). Full-corpus region pass stays manual-only via `scripts/backfill_region.py`. `enrich_pending` gained `item_ids` (`app/services/enrich.py`); `fetch_skipped_bodies` returns `fetched_ids` (`app/services/article_fetch.py`). Typical-night cost ≈ $0.30 WITH vs ~$1,500/yr WITHOUT. See DECISIONS 2026-06-10 Decision 3.
- [x] **Region backfill idempotency — `""` sentinel.** Shipped 2026-06-10. `scripts/backfill_region.py` stores a no-region result as `""` (empty string) instead of `NULL`, so its `region_focus IS NULL` selector no longer re-processes already-evaluated rows; consumers already treat `""` like NULL. Docstring idempotency claim is now true. See DECISIONS 2026-06-10 Decision 4.
- [x] **Remove r/GamesIndustry (frozen feed).** Shipped 2026-06-10. Reddit served a frozen `.rss` (newest 2026-03-23; 0 new items since 2026-05-21) — public-RSS throttling, surfaced as `silent` by the source-health grid. Removed from `sources.yaml` + purged from DB (source id=24 + 13 items + 13 raw_items + 13 enrichments + 5 run_log rows, FK-ordered). NOT `GamesIndustry.biz` (news site, id=9), KEPT. Corpus → 31 active sources. See DECISIONS 2026-06-10 Decision 5.
...
- [x] **Activate scheduler.** Done 2026-06-10. Set `SCHEDULER_ENABLED=1` in `.env` + restarted uvicorn (`--host 127.0.0.1 --port 8001`, no `--reload`). Boot log: `APScheduler started: daily=23:00 local; weekly chained off daily`. Startup-catchup found last daily NOT overdue (<24h) → no immediate run / no surprise spend. First real auto-daily fires tonight 23:00 CST → first real `/runs` cost row. Scheduler now ACTIVE; remaining items are user actions only (watch tonight's run; optional Console spend cap).
- [x] **Harden daily scheduler against Modern Standby misfires.** Done 2026-06-15. Screen-off (S0 Modern Standby) suspended uvicorn and APScheduler's default `misfire_grace_time=1s` discarded the late job (root cause of the missed 2026-06-11 23:00 daily). Cron now uses `misfire_grace_time=None` + targets `run_daily_pipeline_scheduled` (skips if a daily started <12h ago with status running/ok/degraded; a recent `failed` does NOT block). New `POST /runs/trigger-scheduled/daily` endpoint for an optional Windows Task Scheduler poke; `JobRun.triggered_by` documents `'schtask'`. Artifacts `scripts/daily_trigger.xml` + `scripts/trigger_daily.ps1` shipped DEFERRED/UNUSED (in-process cron fired 3/3 nights). 8 new tests in `tests/test_scheduled_guard.py`. See DECISIONS 2026-06-15.
- [ ] **Source CRUD (add / edit / disable / remove) via web forms.** Not started.
- [ ] **Per-source health/error display.** Already partially covered by the Phase 3c.19 source-failure banner; full per-source health page deferred. **Superseded by the operator-console source-grid below** (recency-based, will replace `failing_sources_count` — see DECISIONS 2026-06-09 Decision 3).

### Phase 4 — Reddit RSS rate-limit fix (shipped 2026-06-15)

Reddit's unauthenticated `.rss` endpoint dropped to **1 request / 60s per IP** (single shared bucket across all subreddits) between 06-11 and 06-13 — proven via `x-ratelimit-*` response headers. Serialized reddit fetches behind a 65s rate-gate + a 429 backstop. See DECISIONS 2026-06-15 + SESSION_LOG 2026-06-15.

- [x] **65s process-wide reddit rate-gate.** `_is_reddit_url` + `_reddit_rate_gate` (`threading.Lock` + `monotonic`) in `app/services/scrapers.py`; wired into `fetch_source`'s rss branch. Non-reddit RSS + YouTube unchanged. — **Done 2026-06-15.**
- [x] **429 backstop in `_fetch_reddit_rss`.** Sleeps `x-ratelimit-reset + 2s` (Reddit sends `x-ratelimit-reset`, not `Retry-After`) and retries once. — **Done 2026-06-15.**
- [x] **`/runs` "Ingest only" confirm text "~1 min" → "~10 min".** Reflects the ~10 min idle wall-clock the gate adds for 10 subreddits (no CPU/$). — **Done 2026-06-15.**
- [x] **8 unit tests** `tests/test_reddit_gate.py`. — **Done 2026-06-15.**
- [ ] **Enable the Windows Task Scheduler daily poke (optional).** `scripts/daily_trigger.xml` + `scripts/trigger_daily.ps1` are ready but unused; enable only if a missed-daily recurs despite the `misfire_grace_time=None` fix. (XML-encoding registration failed once — re-check encoding on enable.)

### Phase 4 — Operator console (`/runs`)

**2026-06-09** session: first read-only instrumentation pass on the INERT scheduler — health band, earned `degraded` status, reconcile-on-boot. Scheduler still off; this is what makes activation safe to watch. See SESSION_LOG / DECISIONS / CHANGELOG 2026-06-09.

- [x] **`/runs` health band (read-only operator glance).** `_build_health()` + helpers in `app/routers/runs.py`; band markup in `app/templates/runs.html`; CSS `.gc-health-band` / `.gc-health-card*` / `.gc-health-off` in `app/static/app.css`. 4 cards: Daily pipeline + Weekly extension (latest `JobRun` status + age + live next-run countdown from `app.main._scheduler.get_job(id).next_run_time`; "scheduler off" when unset), Corpus (total items + newest-item age + enrich %), Synthesis (latest synthesized week + current-week synth state). Stale flag daily >25h / weekly >8d. Also: home read-out "Run pipeline" button → "Data flow" link to `/runs`; "Last workflow" callout removed (old `pipeline_run` route + `can_run_pipeline`/`last_workflow` ctx vars left unused). — **Done 2026-06-09.**
- [x] **Earned `degraded` status + floor checks.** `_evaluate_floors()` + `_apply_floor_status()` in `app/services/jobs.py`, wired into `run_daily_pipeline` / `run_weekly_extension` / `run_release_refresh` / `run_ingest_only` / `run_enrich_only`. A run that finishes without raising but under-delivers is downgraded `ok → degraded` (ingest `errors>0`/`fetched==0`; enrich/embed `failed>0`; any non-blocking step — `article_fetch` / `backfill_region` / `release_refresh_*` — recording an `error` key). Trips store `details["_warnings"]` + append `"DEGRADED: ..."`. Rendered as warning chip `.gc-table-chip--warn`; `JobRun.status` doc comment updated. 7-case unit test passes. See DECISIONS 2026-06-09 Decision 1. — **Done 2026-06-09.**
- [x] **Reconcile interrupted runs on boot.** `reconcile_interrupted_runs()` in `app/services/jobs.py`, called from `app/main.py` lifespan after `init_db()` (always). Any `JobRun` still `running` at startup → `failed` + `finished_at` + "interrupted — process exited before completion (auto-reconciled)". Single-process invariant: a `running` row at boot is definitionally dead. Validated in the wild (user-triggered run killed mid-flight → 2 rows auto-reconciled on reload). See DECISIONS 2026-06-09 Decision 2. — **Done 2026-06-09.**
- [x] **Per-source health grid (recency-based).** New `source_health(session)` in `app/services/chrome.py` — one row per source, verdict driven off RunLog recency (latest ingest outcome + items produced in a rolling window): `disabled` / `error` (latest ingest `status='error'`) / `silent` (ran but 0 new items across the whole window) / `idle` (no ingest in 30d, not flagged) / `ok`. `failing_sources_count` rewritten to derive from it (counts `error`+`silent`), replacing the cumulative `error_count > 3` rule — single source of truth for the alert banner AND the grid. Silent-window = `SOURCE_VOLUME_WINDOW_DAYS = 14` (was 7; VG247 false-flag drove the loosen). Rendered as a sorted table (problems first) on `/runs` via a `verdict_chip` macro in `runs.html`, styled in `app.css`. Old `FAILING_SOURCE_ERROR_THRESHOLD = 3` + unused `func` import removed. See DECISIONS 2026-06-09. — **Done 2026-06-09.**
- [x] **`/runs` styling polish + add to sidebar nav.** "Runs" nav item (gauge icon, route `runs_view`) added to `NAV_ITEMS_BASE` in `chrome.py` between Sources and About (is-active on `/runs`). Full CSS pass in `app.css` for the Run-now panel, trigger buttons, status pill, history + source tables, expand `<pre>`, and red/yellow left-border accents on failed/degraded/error/silent rows; uses existing design tokens. — **Done 2026-06-09.**
- [x] **Synthesis retry-with-cap.** `_call_opus` in `app/services/synthesis.py` split into `_opus_once` (single attempt) + bounded-retry `_call_opus`: up to 3 attempts on TRANSIENT failures only (APIConnectionError/timeout, APIStatusError 429/5xx, malformed structured output → `_SynthRetryable`/ValidationError) with exponential backoff (2s, 4s); non-retryable 4xx fail on attempt 1. SDK retries disabled (`with_options(max_retries=0)`) so the app cap is authoritative; after the cap raises ValueError → JobRun `failed` → manual "Synthesis only" re-run. ~37k tokens / ~$0.11 per run; worst case ~3× ≈ $0.33. Unit-tested (success-on-retry / exhaustion / fast-fail). See DECISIONS 2026-06-09. — **Done 2026-06-09.**
- [x] **Cost meter (per-run actual token/$ on `/runs`).** Shipped 2026-06-10. New `app/services/cost.py` — thread-local stack accumulator with EXCLUSIVE (innermost-only) semantics: `record(model, usage)` attributes each Anthropic response's real `usage` (input/output + cache tokens) to the innermost open run, so inline-nested weekly Opus tokens land on the weekly row and aren't double-counted on the daily; thread-confined so a concurrent "skipped" run can't steal tokens (no lock). Priced per model (Haiku $1/$5, Sonnet $3/$15, Opus $4/$20 per MTok; cache read 0.1x, cache write 1.25x; unknown model → $0 + warning). Wired at all 9 Anthropic call sites; `open_run()`/`close_run()` in `jobs._start_run`/`_finish_run` (close folds leaked inner frames). 3 nullable `job_runs` columns (`input_tokens`/`output_tokens`/`cost_usd`) via idempotent `_migrate_job_runs_columns` in `app/db/init.py`; `/runs` Cost column (`$X.XXXX`, tokens in tooltip, `—` when null, colspan 7→8). Verified: imports compile, migration idempotent, stack unit test passes. See DECISIONS 2026-06-10 (cost meter). NOT done (user actions): live `/runs` trigger to populate a real row; scheduler activation.
- [x] **Confirmation dialogs on `/runs` trigger buttons (safety).** Shipped 2026-06-10. All 7 "Run now" buttons gained `hx-confirm="{{ job.confirm }}"` (HTMX-native, no JS build step) stating what the job does + realistic runtime + whether it spends Anthropic $. `app/routers/runs.py` `_TRIGGER_MAP` tuples gained a 4th element (the confirm one-liner), threaded through `trigger_jobs` context (handler unpack → 4-tuple); `app/templates/runs.html` got `hx-confirm` on both button forms. Realistic times from `job_runs` history + operational knowledge (Daily 4–6h, Enrich+embed 3–5h [Whisper], Ingest 10–20m, Cluster 5–15m, Synthesis 5–10m, Weekly 5–10m, Release refresh 2–5m), replacing wrong "~few min" guesses. Two honesty fixes: `cluster_only` *does* spend Sonnet $; `enrich_only`'s hours are Whisper transcription. DELIBERATELY NOT confirmed: eval scoring/note/missing writes (cheap/local/reversible save-on-change) + exec-summary "Generate" (cache-miss-only fraction-of-a-cent + caches forever; `hx-confirm` can't tell cached from uncached → would nag on every free reopen). Verified: 7 `hx-confirm` attrs render live on `/runs`. See DECISIONS 2026-06-10 (confirm dialogs).
- [x] **Alert-banner wording softened.** Shipped 2026-06-10. `app/templates/_alert_banner.html`: "{N} source(s) is/are erroring — click to inspect." → "{N} source(s) needs/need attention — click to inspect." (banner also fires on `silent` sources, so "erroring" overstated it). Singular/plural grammar verified.
