...
- [ ] **Cost meter (per-run actual token/$ on `/runs`).** Designed, NOT built. Read each Anthropic response's real `usage` (input/output/cache tokens), price it per model, accumulate per-run, surface on `/runs` (+ optional soft-cap warning). The one remaining item from the confirmed Phase-4 set.
