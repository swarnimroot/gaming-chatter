...
**Phase 3c.35 shipped (2026-05-27).** 6,958 items · 6,695 Haiku-enriched + embedded (768-dim `nomic-embed-text`) · 184 games tagged · 286+299 authoritative game release dates from pcgamer + IGN (`game_releases` table) · 184+187+189 W19/W20/W21 clusters with Sonnet-4.6 labels · 3 `weekly_reports` rows (W19/W20/W21, all Opus-4.7-synthesized + critic-passed). Weekly read-out at `/` now reads from a precomputed `dashboard_payload_json` cache for 210–240ms per-click renders (~20–25× speedup vs. live compute). Phase 4 automation infrastructure (single APScheduler + `JobRun` table + `/runs` page) is shipped INERT under the `SCHEDULER_ENABLED` env gate. Feature surface unchanged from 3c.23 — exec-summary 1-pager + HTML/PDF export, date-range picker on `/stories` + `/clusters` (vendored flatpickr), bidirectional Trends card (top-5 rising + top-5 declining per tab), per-category `/sentiment` view with right-drawer drill-in, source-failure alert banner when any source's `error_count > 3`, region tabs on `/`/`/stories`/`/clusters`, watch-list category chips on the Watch card, in-app `/eval` harness with critic-feedback loop, welcome modal on `/`. Cumulative LLM spend: ~$20–22. See [`CLAUDE.md`](CLAUDE.md) for per-phase history and [`docs/SESSION_LOG.md`](docs/SESSION_LOG.md) for the latest handoff.
